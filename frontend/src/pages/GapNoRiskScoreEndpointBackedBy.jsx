...
// Auto-generated page for: No /risk-score endpoint backed by ML (rules only)

export default function GapNoRiskScoreEndpointBackedBy() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const res = await fetch('/api/gap-no-risk-score-endpoint-backed-by', {
        method: 'POST',
        headers: {
  ...
  return (
    <div style={{ padding: '2rem', maxWidth: '900px', margin: '0 auto' }}>
      <h1 style={{ fontSize: '1.75rem', marginBottom: '0.5rem' }}>No /risk-score endpoint backed by ML (rules only)</h1>
      <p style={{ color: '#666', marginBottom: '1.5rem' }}>No /risk-score endpoint backed by ML (rules only)</p>
      <form onSubmit={handleSubmit} style={{ marginBottom: '1.5rem' }}>
        ...
        >
          {loading ? 'Processing...' : 'Run No /risk-score endpoint backed by ML (rules only)'}
        </button>
      </form>
